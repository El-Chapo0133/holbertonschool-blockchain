 ...
 * ==========================================================================
 *
 *       Filename:  undate_unspent.c
 *
 *    Description:   updates the list of all unspent transaction outputs
 *			given a list of processed transactions
 *
 *        Version:  1.0
 *        Created:  04/14/25 10:36:28
 *       Revision:  none
 *       Compiler:  gcc
 *
...
int foreach_input(llist_node_t node, unsigned int idx, void *_args)
{
	...
	llist_remove_node(args[0], find_unspent,
			((tx_in_t *)node)->tx_out_hash, 1, free);
	return (0);
	(void)idx;
	(void)args;
}
...
int foreach_output(llist_node_t node, unsigned int idx, void *_args)
{
	...
	if (!utxo)
	{
		fprintf(stderr, "foreach_output: failed to create UTXO.\n");
		exit(1);
	}

	if (llist_add_node(args[0], utxo, ADD_NODE_REAR))
	{
		fprintf(stderr, "foreach_output: failed to add node.\n");
		exit(1);
	}
	return (0);
	(void)idx;
